...
//   set_vector(floppy_ISR, M_VEC+6, (D_INT+D_PRESENT+D_DPL1));
   //enable_irq(6);
	IRQ::install_handler( 6, FloppyIRQ );
...
//   set_vector(_int1c, 0x1c, (D_INT+D_PRESENT+D_DPL1));
   ...
   cout<<"Initialising Floppy driver...  \n";
	switch(i)
	{
		case 0xFF: cout<<" [ controller not found ]\n";
				break;
		case 0x80: cout<<" [ NEC controller ]\n";
				break;
		case 0x81: cout<<" [ VMware controller ]\n";
				break;
		case 0x90: cout<<" [ enhanced controller ]\n";
				break;
		default: cout<<" [ unknown controller ["<<i<<"] ]\n";
	}
   /*if (i == 0x80)
...
bool waitfdc(bool sensei)
{
   tmout = 100;   /* set timeout to 1 second */
#ifdef DEBUG
   cout<<"waitfdc start\n";
#endif   
   ...
   while ( !done && tmout )
	{
		my_timer->sleep(tmout);
		tmout=0;
	}
     
#ifdef DEBUG
   cout<<"waitfdc !done && tmout\n";
#endif
...
#ifdef DEBUG
	cout<<"clearing disk change status...\n";
#endif
...
void motoron(void)
{
   if (!motor) {
      ...
      my_timer->sleep( 500 ); /* delay 500ms for motor to spin up */
      motor = true;
   }
}
...
{
#ifdef DEBUG
	cout<<"recalibrating, motor spin up...\n";
#endif
...
#ifdef DEBUG
	cout<<"recalibrating, wait for seek...\n";
#endif
...
bool fdc_rw(int block,BYTE *blockbuff,bool read,unsigned long nosectors)
{
   ...
   for (tries = 0;tries < 3;tries++) {
	 ...
	 recalibrate();
	 motoroff();
	 cout<<"FDC: Disk change detected. Trying again.\n";
	 ...
	 return fdc_rw(block, blockbuff, read, nosectors);
      ...
      if (!flseek(track)) {
	 motoroff();
	 cout<<"FDC: Error seeking to track\n";
	 return false;
      }
	...
	sendbyte(DG168_GAP3RW);  /* gap 3 size for 1.68M red/write */
      sendbyte(0xff);            /* DTL = unused */
      
      /* wait for command completion */
      /* read/write don't need "sense interrupt status" */
      if (!waitfdc(true)) {
	cout<<"Timed out, trying operation again after reset()\n";
	reset();
	return fdc_rw(block, blockbuff, read, nosectors);
      }
